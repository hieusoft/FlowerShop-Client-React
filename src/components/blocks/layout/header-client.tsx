...
export function HeaderClient({ occasions }: { occasions: React.ReactNode }) {
  ...
  const [notifyQty, setNotifyQty] = useState(0);
        ...
        <HeaderSearch>
          ...
          >
            ...
            <div className={cn("flex gap-2 ml-auto", isMobile ? "" : "hidden")}>
              <Button
                variant="ghost"
                className="bg-background border rounded-full size-12 p-0"
            ...
            >
              ...
              >
                <div
                  className={cn("flex gap-2 ml-auto", isMobile ? "" : "hidden")}
                >
                  <Button
                    variant="ghost"
                    className="bg-background rounded-full size-12 p-0"
                    onClick={handleMobileExpandedToggle}
                  >
                ...
                <NavigationMenuItem value="header-bouquets">
                  <NavigationMenuTrigger
                    className={isMobile ? "rounded-full size-10 p-0" : ""}
                    hasIcon={!isMobile}
                  >
                    {isMobile ? (
                ...
                <HeaderSearchTrigger>
                  <Button
                    variant="ghost"
                    className="bg-background rounded-full size-10 p-0"
                  >
                ...
                <NavigationMenuItem className="relative">
                  ...
                  >
                    ...
                    {cartQty > 0 && (
                      <Badge
                        variant="secondary"
                        className="absolute -bottom-1 -right-1 h-5 min-w-5 p-1 tabular-nums"
                      >
                        {cartQty}
                      </Badge>
                    )}
                  ...
                  <NavigationMenuContent>
                    <div className="flex flex-col md:flex-row md:justify-end md:text-end gap-4 w-full">
                      <div className="flex flex-col">
                        {cartQty === 0 ? (
                          <Empty className="gap-2 max-h-none py-0 px-2 items-start">
                            <EmptyHeader className="font-heading text-2xl py-1">
                              Your basket is empty
                            </EmptyHeader>
                            <EmptyContent>
                              Things you add into your basket will be displayed
                              here.
                            </EmptyContent>
                          </Empty>
                        ) : (
                          <p className="px-2 py-2">
                            You have {cartQty} item(s) in your cart.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row md:justify-end md:text-end gap-4 w-full">
                      <div className="flex flex-col">
                        {notifyQty === 0 ? (
                          ...
                          <p className="px-2 py-2">
                            You have {notifyQty} notification(s).
                          </p>
                        )}
                      </div>
                    </div>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent className="flex flex-col md:flex-row md:justify-end md:text-end">
                    ...
                    <div className="flex flex-col md:items-end md:text-end">
                      <h3 className="font-heading tracking-tight px-2 py-1 text-2xl">
                        Colors
                      </h3>
                      <ToggleGroup
